...
def get_identificazione_by_id(identificazione_id: int, db: Session = Depends(get_db)):
    ...
    """
    ident = (
        db.query(IdentificazioneSpecieDB)
        .filter(IdentificazioneSpecieDB.id == identificazione_id)
        .first()
    )
...
def create_identificazione(
    identificazione: IdentificazioneSpecieCreate, db: Session = Depends(get_db)
):
    """
...
def add_feedback(identificazione_id: int, feedback: str, db: Session = Depends(get_db)):
    """
...
def get_identificazioni_stats(db: Session = Depends(get_db)):
    ...
    identificate = (
        db.query(IdentificazioneSpecieDB)
        .filter(IdentificazioneSpecieDB.status == SpecieStatus.IDENTIFICATA)
        .count()
    )
    non_identificate = (
        db.query(IdentificazioneSpecieDB)
        .filter(IdentificazioneSpecieDB.status == SpecieStatus.NON_IDENTIFICATA)
        .count()
    )
    in_attesa = (
        db.query(IdentificazioneSpecieDB)
        .filter(IdentificazioneSpecieDB.status == SpecieStatus.IN_ATTESA)
        .count()
    )
