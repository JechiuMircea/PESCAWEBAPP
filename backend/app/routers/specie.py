...
from ..database import get_db, SpecieItticaDB
from ..models import SpecieIttica, SpecieItticaCreate, SpecieItticaUpdate
...
def create_specie(specie: SpecieItticaCreate, db: Session = Depends(get_db)):
    ...
    existing_specie = db.query(SpecieItticaDB).filter(
        SpecieItticaDB.nome_scientifico == specie.nome_scientifico
    ).first()
    
    if existing_specie:
        raise HTTPException(
            status_code=400, 
            detail="Esiste già una specie con questo nome scientifico"
        )
...
def update_specie(
    specie_id: int, 
    specie_update: SpecieItticaUpdate, 
    db: Session = Depends(get_db)
):
    """
...
def search_specie(query: str, db: Session = Depends(get_db)):
    ...
    """
    specie = db.query(SpecieItticaDB).filter(
