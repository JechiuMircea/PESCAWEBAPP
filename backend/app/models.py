...
class SpecieIttica(BaseModel):
    ...
    periodo_riproduzione: Optional[str] = Field(None, description="Periodo di riproduzione")
...
class IdentificazioneSpecie(BaseModel):
    ...
    utente_id: Optional[str] = Field(None, description="ID dell'utente che ha richiesto l'identificazione")
    specie_identificata: Optional[str] = Field(None, description="Specie identificata dal sistema ML")
    confidence_score: Optional[float] = Field(None, description="Punteggio di confidenza dell'identificazione")
    status: SpecieStatus = Field(default=SpecieStatus.IN_ATTESA, description="Status dell'identificazione")
    feedback_utente: Optional[str] = Field(None, description="Feedback dell'utente sull'identificazione")
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: datetime = Field(default_factory=datetime.now)
