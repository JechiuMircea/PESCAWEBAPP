...
def test_specie_endpoints():
    ...
    if response.status_code == 200:
        ...
        if specie:
            ...
            print(
                f"   Prima specie: {prima_specie['nome_comune']} ({prima_specie['nome_scientifico']})"
            )
    else:
...
def test_identificazioni_endpoints():
    ...
    response = requests.post(f"{BASE_URL}/identificazioni/", json=test_data)
...
def main():
    ...
    try:
        ...
        print(
            "   Assicurati che l'API sia in esecuzione con: uvicorn backend.app.main:app --host 127.0.0.1 --port 8080"
        )
